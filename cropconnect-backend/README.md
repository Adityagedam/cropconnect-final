...
- Set `AI_PROVIDER=openai` with `OPENAI_API_KEY`, or `AI_PROVIDER=gemini` with `GEMINI_API_KEY`, for AI chat answers, crop planning, translations, and AI market advice. Set `GOOGLE_API_KEY` and `GOOGLE_CSE_ID` only to add Google Custom Search snippets to the AI prompt.
